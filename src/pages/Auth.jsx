    ...
    return(
        <main className='h-full flex justify-center items-center flex-col gap-6 px-4 py-8'>
            ...
            <section className='bg-white-surface flex justify-center items-center flex-col w-full max-w-[420px] relative shadow-[0_30px_70px_-25px_rgba(23,26,24,0.28)] rounded-[24px] border border-white-borders p-6 md:p-8'>
                <h1 className="font-semibold text-2xl text-white-text mb-6">{signUp ? 'Sign up' : 'Log in'}</h1>
        <form className="flex justify-start items-center gap-4 flex-col w-full">
            <div className="flex justify-center flex-col gap-3 w-full">
                ...
                <button className="mt-1 w-full rounded-xl bg-white-accent px-4 py-3 text-base font-semibold text-white-background shadow-[0_18px_30px_-18px_rgba(8,120,23,0.8)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_22px_32px_-18px_rgba(8,120,23,0.9)]">{signUp ? 'Sign up' : 'Log in'}</button>
            </div>
           ...
           <p className="text-sm text-white-text/80">{signUp ? 'Already have an account?' : 'Dont have an account?'}<span className="font-semibold text-white-accent cursor-pointer" onClick={()=>{
                setSignUp(!signUp);
            }}>{signUp ? 'Log in' : 'Sign up'}</span></p>
        </form>
        </section>
        </main>
    )
